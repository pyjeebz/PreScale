"""Agent management commands for Prescale CLI."""
...
@click.group()
def agent():
    """Manage Prescale metrics collection agents."""
    pass
...
def agent_status(ctx: click.Context):
    ...
    console.print("[bold]Prescale Agent Status[/bold]\n")
    ...
    # Available sources
    try:
        from prescale_agent.sources import list_sources
        sources = list_sources()
        console.print(f"\n[bold]Available Source Types:[/bold] {', '.join(sources)}")
    except ImportError:
        console.print("\n[yellow]prescale-agent not installed. Install with:[/yellow]")
        console.print("  pip install prescale-agent")
...
def agent_sources():
    ...
    try:
        ...
        source_info = {
            ...
            "datadog": ("Datadog API", "pip install prescale-agent[datadog]"),
            "cloudwatch": ("AWS CloudWatch", "pip install prescale-agent[aws]"),
            "azure_monitor": ("Azure Monitor", "pip install prescale-agent[azure]"),
            "gcp_monitoring": ("Google Cloud Monitoring", "pip install prescale-agent[gcp]"),
        }
        ...
        console.print("[yellow]prescale-agent not installed. Install with:[/yellow]")
        console.print("  pip install prescale-agent")


@agent.command("init")
@click.option("--output", "-o", type=click.Path(), default="prescale-agent.yaml", help="Output file path")
def agent_init(output: str):
    """Generate a sample agent configuration file."""
    try:
        from prescale_agent.cli_v2 import init as agent_init_cmd
        # Invoke the agent's init command
        import subprocess
        import sys
        result = subprocess.run(
            [sys.executable, "-m", "prescale_agent.cli_v2", "init", "-o", output],
            capture_output=True,
            text=True,
        )
...
"""
        ...
        console.print("\nRun the agent with:")
        console.print(f"  [cyan]prescale-agent run -c {output}[/cyan]")
...
def agent_test(config_path: str | None):
    ...
    try:
        ...
        cmd = [sys.executable, "-m", "prescale_agent.cli_v2", "test"]
        if config_path:
        ...
        console.print("\n[yellow]Make sure prescale-agent is installed:[/yellow]")
        console.print("  pip install prescale-agent")

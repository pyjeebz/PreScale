...
def collect():
    ...
    try:
        ...
        r.raise_for_status()
        # Try JSON first (Locust can return JSON for /stats/requests)
        parsed = None
        try:
            parsed = r.json()
        except Exception:
        ...
        if isinstance(parsed, dict) and parsed.get("stats"):
            for stat in parsed.get("stats", []):
                ...
                total = float(stat.get("num_requests") or stat.get("num_requests", 0) or 0)
                failures = float(stat.get("num_failures") or 0)
                avg = float(stat.get("avg_response_time") or stat.get("avg") or 0)
                rps = float(stat.get("current_rps") or stat.get("current_rps", 0) or 0)
            ...
            # Fallback: HTML table parsing (legacy)
